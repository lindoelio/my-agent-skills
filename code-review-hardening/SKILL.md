...
Apply direct fixes autonomously. Max 2 passes.
...
- Repeat Pass 1 on remaining unfixed `direct-fix` items.
- If a Pass 2 fix fails (introduces new blocking finding): rollback,
  mark as `author-required`.

After 2 passes, any remaining `direct-fix` items are marked
